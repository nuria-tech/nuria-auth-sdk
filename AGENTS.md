...
- `isImpersonating()` — returns `true` when the current in-memory session token carries an `act` claim (i.e. was minted via the support impersonation flow). Safe to call when unauthenticated (returns `false`). Portals use this to render the operator-identity banner without parsing the JWT directly. Guards against claim-confusion attacks: only treats `act` as meaningful when `sub` differs from `act.sub`; if they match (self-delegation) returns `false`.
- `AuthConfig.onSessionInvalidated` — optional callback fired **after** a permanent server-side refresh rejection (`invalid_grant`, revoked token, DPoP mismatch). Fires after the in-memory session is cleared and `onAuthStateChanged(null)` is sent so the portal can surface a "sua sessão expirou" toast. Not called for network errors or 5xx (those are retried) — only for definitive server rejections that make the refresh token permanently unusable.
