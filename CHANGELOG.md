...
- Re-tag release to trigger npm CI publish (v9.1.4 tag was pushed alongside
  many other tags simultaneously, which prevented the GitHub Actions workflow
  from firing).
...
- Style-only: reformatted long `if` condition in `init()` for readability — no behaviour change.
